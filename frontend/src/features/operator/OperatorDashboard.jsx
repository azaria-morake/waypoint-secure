...
const OperatorDashboard = () => {
  ...
  // UI State
  const [activeCardId, setActiveCardId] = useState(null); // Which card is expanded
  const [callingContact, setCallingContact] = useState(null); // Who are we calling?
  const [pinnedMessage, setPinnedMessage] = useState(null); // Toast message

  // Sorting: Critical incidents top
  ...
  const handleCall = (name, number) => {
    ...
    // Auto hangup simulation after 3s
    ...
    <div className="w-full h-full flex relative">
      ...
      {callingContact && (
        <div className="absolute inset-0 z-50 bg-black/80 flex items-center justify-center backdrop-blur-sm animate-in fade-in">
          <div className="bg-slate-900 border border-slate-700 p-8 rounded-2xl shadow-2xl text-center max-w-sm w-full">
            ...
            <p className="text-slate-400 font-mono mb-6">{callingContact.number}</p>
            <button 
      ...
      <aside className="w-80 bg-slate-900 border-r border-slate-800 flex flex-col flex-none z-20 shadow-xl">
        ...
        {sortedJourneys.map((j) => {
            ...
            // 🛠️ FIX: Check for both 'critical' AND 'panic_dispatched'
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 pt-0 text-sm border-t border-slate-700/50 mt-1 animate-in slide-in-from-top-2">
                    ...
                    {/* Contacts (Dialable) */}
                    <div>
                        <label className="text-[10px] text-slate-500 block mb-1">EMERGENCY CONTACTS</label>
                        <div className="space-y-1">
                            {(j.bestContacts || []).map((contact, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-slate-900/50 p-2 rounded border border-slate-700">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-slate-500 text-[10px] font-mono">{contact.phone}</span>
                                    </div>
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); handleCall(contact.name, contact.phone); }}
                            ...
                            {(!j.bestContacts || j.bestContacts.length === 0) && (
                                <div className="text-slate-600 text-xs italic p-2">No contacts linked</div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4 pt-3 border-t border-slate-700">
                         <button 
                            onClick={(e) => { e.stopPropagation(); handleCall(j.fullName, j.contactNumber); }}
                            className="flex-1 bg-slate-700 hover:bg-green-600 hover:text-white text-slate-300 py-2 rounded text-xs font-bold transition-colors flex items-center justify-center gap-2"
                        >
